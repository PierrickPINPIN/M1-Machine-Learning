...
## Question 19
```{r}
newspamdata <- spamdata
newspamdata$spam <- as.factor(newspamdata$spam)
newspamdata
```
...
sample_size <- sample(1:nrow(spamdata), size= round(0.8*nrow(newspamdata)), replace= FALSE)

newtraining_set <- newspamdata[sample_size,]
newtest_set <- newspamdata[-sample_size,]
...
ltune <- train(spam~., data=newtraining_set, method= "glm")

print(ltune)
```

```{r}
print(ltune$finalModel)
```

```{r}
summary(ltune$finalModel)
```
...
```{r}
ltpre <- predict(ltune$finalModel, newtest_set, type= "response")

ltacc <- Accuracy(ltpre, newtest_set$spam)
ltacc
```
>We had seen that the precision of the logistic model was 0.925. With the tuning, we obtain the same result.
...
btune <- train(spam~., data=newtraining_set, method= "treebag")

print(btune)
```

```{r}
print(btune$finalModel)
```

```{r}
plot(varImp(btune))
```
>We had seen that the accuracy of the bagged model was 0.9130435. With the tuning, we get an accuracy of 0.9308196. The tuning allowed us to improve the model.
...
rftune <- train(spam~., data=newspamdata, method= "rf")
...
plot(rftune)
```
>We had seen that the precision of the random forest was 0.951087. With the tuning, we get an accuracy of 0.9442497. The result found is less accurate. The mtry parameter was 7.549834, with the tuning the best mtry parameter is 29. It is therefore the mtry parameter that explains this difference.
...
bootune <- train(spam~., data=newspamdata, method= "gbm")
```

```{r}
print(bootune$results)
```
...
```{r}
print(bootune$finalModel)
```
```{r}
print(bootune)
```
>We had seen that the accuracy of the boosted model was 0.951087. With the tuning, we obtain an accuracy of 0.9457970. The number of n.trees was 500 without tuning against 150 with tuning. We can see that the more this parameter increases, the more the precision increases.
